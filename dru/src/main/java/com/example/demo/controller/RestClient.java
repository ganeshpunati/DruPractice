...
import com.example.demo.JsonTopic;
...
//		 restTemplate.exchange
	...
	private static void callGetByIdApi() {
		...
		//from api response
		HttpEntity<String> entity = new HttpEntity<>("parameters", headers);
		 JsonTopic result = restTemplate.getForObject(Get_BY_ID_API, JsonTopic.class,param);
	...
	public static void callCreateUserAPI() {
		JsonTopic jsonTopic = new JsonTopic(500,5000,"new data",true);
		ResponseEntity<JsonTopic> result = restTemplate.postForEntity(CREATE_API, jsonTopic, JsonTopic.class);
		System.out.println("*************************");
